{{-- ── Create Bank Type Modal ── --}}

<div class="modal fade" id="createBankTypeModal" tabindex="-1" aria-labelledby="createBankTypeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow-lg border-0" style="border-radius: 16px; overflow: hidden;">
            <form method="POST" action="{{ route('settings.bank-types.store') }}">
                @csrf
                <div class="modal-header border-bottom-0 pt-4 px-4 pb-0">
                    <h5 class="modal-title fw-bold" id="createBankTypeModalLabel" style="color: #111827; letter-spacing: -0.025em; font-size: 1.25rem;">Add Bank Type</h5>
                ...
                <div class="modal-body px-4 pt-2">
                    <p class="text-muted mb-4" style="font-size: 0.9rem; line-height: 1.5;">Register a new bank type into the system.</p>
                ...
                <div class="modal-footer border-top-0 px-4 pb-4 pt-2" style="background-color: #ffffff;">
                    ...
                    <button type="submit" class="btn text-white d-inline-flex align-items-center gap-2" style="background-color: {{ config('brand.primary_color') }}; font-weight: 600; font-size: 0.875rem; border-radius: 8px; padding: 10px 20px; border: none;">
                        <i class="fas fa-save"></i> Save Bank Type
                    </button>
                </div>
            </form>
        </div>
...
@if($errors->any() && old('_method') !== 'PUT')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var createModal = new bootstrap.Modal(document.getElementById('createBankTypeModal'));
            createModal.show();
        });
    </script>
@endif

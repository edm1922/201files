...
<li class="ui-tree-node">
    <div class="ui-tree-item group d-flex align-items-center gap-2 py-1 px-2 mb-1 rounded-2 transition-all {{ $isActive ? 'bg-light active' : '' }}">
        
        {{-- Chevron --}}
        @if($children->isNotEmpty())
            <a data-bs-toggle="collapse" href="#folder-collapse-{{ $folder->id }}" role="button" aria-expanded="true" class="text-decoration-none d-flex align-items-center justify-content-center" style="width: 16px;">
                <i class="fas fa-chevron-down text-muted opacity-50 flex-shrink-0 collapse-icon" style="font-size: 0.65rem;"></i>
            </a>
        @else
        ...
        @if($isActive && $children->isNotEmpty())
            <a data-bs-toggle="collapse" href="#folder-collapse-{{ $folder->id }}" role="button" aria-expanded="true"
               class="d-flex align-items-center gap-2 text-decoration-none flex-grow-1 overflow-hidden" style="min-width: 0;">
        @else
            <a href="{{ route('department-documents.index', ['department_id' => $selectedDepartmentId, 'document_folder_id' => $folder->id]) }}" 
               class="d-flex align-items-center gap-2 text-decoration-none flex-grow-1 overflow-hidden" style="min-width: 0;">
        @endif
           
            <i class="fas {{ $isOpen ? 'fa-folder-open text-danger' : 'fa-folder text-secondary opacity-75' }}" style="font-size: 1.1rem; width: 20px; text-align: center;"></i>
            
            <span class="font-monospace text-truncate {{ $isActive ? 'text-dark fw-bold' : 'text-secondary' }}" style="font-size: 0.85rem;">
                {{ $folder->name }}
            </span>
            @if ($folder->folder_code)
                <span class="text-muted small ms-auto me-2" style="font-size: 0.75rem;">{{ $folder->folder_code }}</span>
            @endif
        </a>
        ...
        @if ($canManageFolders ?? false)
            <div class="ui-tree-actions d-flex align-items-center gap-1 ms-auto ps-2">
                <button type="button" class="btn btn-sm btn-link text-danger p-0 text-decoration-none" title="Add Subfolder"
                    data-bs-toggle="modal" data-bs-target="#createDepartmentFolderModal"
                    data-folder-department-id="{{ $selectedDepartmentId }}"
                    data-folder-parent-id="{{ $folder->id }}"
                    data-folder-create-scope="New folder inside {{ $folder->name }}">
                    <i class="fas fa-plus bg-danger text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 18px; height: 18px; font-size: 0.6rem;"></i>
                </button>
                
                <div class="dropdown">
                    <button class="btn btn-sm btn-link text-secondary p-0 text-decoration-none shadow-none" type="button" data-bs-toggle="dropdown" aria-expanded="false" title="Folder actions">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 rounded-3">
                        <li>
                            <button type="button" class="dropdown-item d-flex align-items-center gap-2 py-2"
                                ...
                                data-folder-code="{{ $folder->folder_code }}"
                                data-folder-action="{{ route('department-documents.folders.update', $folder) }}">
                                <i class="fas fa-pen text-secondary" style="width: 16px;"></i><span class="fw-medium">Rename</span>
                            </button>
                        </li>
                        <li>

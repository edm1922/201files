    ...
    <div x-data="locationManager()">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted mb-0" style="font-size: 0.85rem;">Manage the physical cabinets and racks where
        ...
        <div class="d-flex flex-column gap-3 mb-5">
            @forelse($locations as $cabinetId => $racks)
                ...
                <div class="card shadow-sm border-0" style="border-radius: 12px; overflow: hidden;"
                    ...
                    <div class="card-header bg-white border-bottom-0 p-4 d-flex justify-content-between align-items-center"
                        style="cursor: pointer; transition: background 0.2s;" @click="expanded = !expanded"
                        onmouseover="this.style.backgroundColor='#f9fafb'"
                        onmouseout="this.style.backgroundColor='#ffffff'">

                        <div class="d-flex align-items-center gap-3">
                            <div class="d-flex align-items-center justify-content-center"
                                style="width: 48px; height: 48px; border-radius: 12px; background: rgba(221, 39, 13, 0.08);">
                            ...
                            <div>
                                ...
                                <div class="d-flex gap-3 text-muted" style="font-size: 0.85rem;">
                                    <span><i class="fas fa-layer-group me-1 opacity-50"></i> {{ $rackCount }}
                                        {{ Str::plural('Rack', $rackCount) }}</span>
                                    <span><i class="fas fa-folder me-1 opacity-50"></i> {{ $totalFolders }}
                                        {{ Str::plural('Folder', $totalFolders) }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <span class="btn btn-light rounded-circle"
                                style="width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center; color: #6b7280; transition: transform 0.3s;"
                    ...
                    <div x-show="expanded" x-collapse style="display: none;" class="border-top">
                        <div class="p-0">
                            <table class="table table-hover mb-0 align-middle" style="font-size: 0.9rem;">
                                <thead style="background-color: #f9fafb;">
                                    <tr>
                                        <th class="border-0 text-uppercase text-muted"
                                            style="font-size: 0.75rem; font-weight: 600; letter-spacing: 0.05em; padding: 12px 24px;">
                                            Rack ID</th>
                                        <th class="border-0 text-uppercase text-muted text-center"
                                            style="font-size: 0.75rem; font-weight: 600; letter-spacing: 0.05em; padding: 12px 24px; width: 150px;">
                                            Folders</th>
                                        <th class="border-0 text-uppercase text-muted text-center"
                                            style="font-size: 0.75rem; font-weight: 600; letter-spacing: 0.05em; padding: 12px 24px; width: 100px;">
                                            Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            <td class="border-bottom-0" style="padding: 16px 24px;">
                                                <div class="d-flex align-items-center gap-2">
                                                    <div
                                                        style="width: 6px; height: 6px; border-radius: 50%; background-color: #10b981;">
                                                    </div>
                                                    <span class="fw-semibold"
                                                        style="color: #374151;">{{ $location->rack_id }}</span>
                                                </div>
                                            </td>
                                            <td class="border-bottom-0 text-center" style="padding: 16px 24px;">
                                                <span class="badge rounded-pill"
                                                    style="background: rgba(79, 70, 229, 0.1); color: #4f46e5; padding: 5px 12px; font-weight: 600; font-size: 0.8rem;">
                                                    <i class="fas fa-folder me-1" style="font-size: 0.6rem;"></i>
                                                    {{ $location->employees_count + $location->departments_count }}
                                                </span>
                                            </td>
                                            <td class="border-bottom-0 text-center" style="padding: 16px 24px;">
                                                <div class="d-flex justify-content-center gap-2">
                                                    ...
                                                    <button type="button" class="btn btn-sm"
                                                        ...
                                                        @if ($location->employees_count + $location->departments_count == 0) onmouseover="this.style.backgroundColor='#fee2e2'; this.style.transform='scale(1.05)'"
                                                                onmouseout="this.style.backgroundColor='#fef2f2'; this.style.transform='scale(1)'"
                                                                @click="openConfirmModal({{ $location->id }}, '{{ $location->cabinet_id }} › {{ $location->rack_id }}')" @endif>
            ...
            @empty
                <div class="card shadow-sm border-0"
                    style="border-radius: 12px; border-left: 4px solid #9ca3af !important;">
                    <div class="card-body text-center py-5">
                        ...
                        <p class="text-muted mb-0" style="font-size: 0.9rem;">Start by adding a new physical location
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('locationManager', () => ({
                ...
                openConfirmModal(id, name) {
                    ...
                    this.confirmMessage =
                        `Are you sure you want to permanently delete the rack <strong>${name}</strong>? This action cannot be undone.`;
